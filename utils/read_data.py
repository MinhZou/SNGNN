...
def read_planetoid_data_SGGNN(folder, prefix):
    ...
    if prefix.lower() == 'citeseer':
        ...
        # index = torch.randint(0, tx.size(1), (1, len_test_indices))[0]
        # tmp_feature = torch.eye(tx.size(1), tx.size(1))
        # tx_ext = tmp_feature[index]
        tx_ext[sorted_test_index - test_index.min(), :] = tx
        ty_ext = torch.zeros(len_test_indices, ty.size(1))
        # index = torch.randint(0, ty.size(1), (1, len_test_indices))[0]
        # tmp_label = torch.eye(ty.size(1), ty.size(1))
        # ty_ext = tmp_label[index]
